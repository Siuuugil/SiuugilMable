...
const int mapSize = 20;  // 맵 크기
const int squareSize = 80; // 각 칸의 크기
...
struct Player {
    ...
    int collateral;      // 담보
    int loanTerm;        // 대출 만기
    std::vector<int> ownedProperties; // 소유한 부동산 목록을 추가합니다
};
...
std::vector<Zone> zones = {
    // 위쪽 줄 (왼쪽에서 오른쪽으로)
    // 위쪽 줄 (아시아 주요 도시들)
    {0, 0, squareSize, squareSize, L"서울", RGB(255, 0, 0), 1000, -1},
    ...
    {squareSize * 4, 0, squareSize * 5, squareSize, L"쿠알라룸푸르", RGB(0, 255, 255), 800, -1},
    ...
    {squareSize * 8, 0, squareSize * 9, squareSize, L"싱가포르", RGB(0, 0, 128), 880, -1},
    {squareSize * 9, 0, squareSize * 10, squareSize, L"감옥", RGB(192, 192, 192), 0, -1},
    ...
    {0, squareSize * 9, squareSize, squareSize * 10, L"출발!", RGB(64, 224, 208), 0, -1},
...
LRESULT CALLBACK IngameWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {   
        ...
        HWND D_Button2 = CreateWindow(L"BUTTON", L"대출 실행", WS_CHILD | WS_VISIBLE, 1150, 300, 100, 50, hWnd, (HMENU)3, hInst, NULL);
        HWND H_Button2 = CreateWindow(L"BUTTON", L"대출 상환", WS_CHILD | WS_VISIBLE, 1280, 300, 100, 50, hWnd, (HMENU)4, hInst, NULL);
    ...
    case WM_PAINT:
    {
        ...
        Rectangle(hdc, sugilBank.rect.left, sugilBank.rect.top-45, sugilBank.rect.right + 400, sugilBank.rect.bottom-120);
        ...
        TextOut(hdc, sugilBank.rect.left + 400, sugilBank.rect.top + 80, loanText4, lstrlenW(loanText4));
        swprintf_s(loanText5, L"플레이어 2의\n담보: $%d", players[0].collateral);
        TextOut(hdc, sugilBank.rect.left + 400, sugilBank.rect.top + 110, loanText5, lstrlenW(loanText5));
        swprintf_s(loanText6, L"플레이어 2의\n대출 만기: %d턴", players[0].loanTerm);
        TextOut(hdc, sugilBank.rect.left + 400, sugilBank.rect.top + 140, loanText6, lstrlenW(loanText6));

        //플레이어 1 정보 
        Rectangle(hdc, player1Info.rect.left, player1Info.rect.top+400,
            player1Info.rect.right, player1Info.rect.bottom+400);
        TextOut(hdc, player1Info.rect.left + 10, player1Info.rect.top + 410,
        ...
        // P2 정보창 그리기
        Rectangle(hdc, player2Info.rect.left, player2Info.rect.top+400,
            player2Info.rect.right, player2Info.rect.bottom+400);
        TextOut(hdc, player2Info.rect.left + 10, player2Info.rect.top + 410,
...
void executeLoan(int playerIndex) {
    ...
    if (players[playerIndex].loanAmount == 0) {
        ...
        WCHAR message[100];
        swprintf_s(message, L"Player %d님은  $%d을 빌리셨습니다.", playerIndex + 1, LOAN_AMOUNT);
        MessageBox(NULL, message, L"반드시 갚아라", MB_OK);
    }
    else {
